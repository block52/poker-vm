...
const VacantPlayer: React.FC<VacantPlayerProps> = memo(
  ({ left, top, index }) => {
    const { isUserAlreadyPlaying, tableInfo, isSeatVacant: checkSeatVacant, canJoinSeat: checkCanJoinSeat } =
    ...
    const handleConfirmSeat = async () => {
      ...
      const waitForBB = JSON.parse(localStorage.getItem("wait_for_big_blind") || "true");
      ...
      try {
        ...
        setTimeout(() => {
          if (userAddress) refreshNonce(userAddress);
        }, 1000);
      } catch (err) {
            ...
            <div className="bg-gradient-to-b from-gray-800 to-gray-900 p-8 rounded-xl shadow-2xl w-96 border border-gray-700 relative">
              ...
              <div className="flex justify-between space-x-4 mt-6">
                <button
                  ...
                  className="px-5 py-3 bg-gray-600 text-white rounded-lg flex-1"
                >
                  No
                </button>
                <button
  ...
  (prev, next) =>
    prev.left === next.left && prev.top === next.top && prev.index === next.index
);
