...
const Table = React.memo(() => {
    ...
    const winningSeat = hasWinner ? winnerInfo[0].seat : null;
    const winningAmount = hasWinner ? winnerInfo[0].formattedAmount : "0";

    const [showChip, setShowChip] = useState(false);
    const [moveChip, setMoveChip] = useState(false);

    useEffect(() => {
        if (hasWinner) {
            // 1) Immediately show the chip in the pot
            setShowChip(true);

            // 2) After 300ms, switch to showing the chip in the seat (fires layout animation)
            const toSeat = setTimeout(() => {
                setMoveChip(true);
            }, 300);

            // 3) After 1000ms total (enough for pot→seat + a pause), remove everything
        ...
        // If hasWinner flips to false, clear any timers:
        return undefined;
    }, [hasWinner]);
                                    ...
                                    <div className="w-[900px] h-[450px] relative text-center block transform translate-y-[30px]">
                                        <div className="h-full flex align-center justify-center">
                                            <div className="z-20 relative flex flex-col w-[900px] h-[350px] left-1/2 top-0 transform -translate-x-1/2 text-center border-[3px] border-rgba(255, 255, 255, 0.2) border-solid rounded-full items-center justify-center shadow-[0_7px_15px_rgba(0,0,0,0.6)]">
                                                ...
                                                <div className="flex flex-col items-center justify-center -mt-20">
                                                    <div className="pot-display" style={{ position: "relative" }}>
                                                        Total Pot:
                                                        ...
                                                        {/* ─── POT‐CHIP: only when showChip is true AND moveChip is false ─── */}
                                                        <AnimatePresence>
                                                            {showChip && !moveChip && (
                                                                <motion.div
                                                                    key="potChip"
                                                                    layoutId="winningChip"
                                        ...
                                        <div className="absolute inset-0 z-30">
                                            {reorderedPlayerArray.map((position, positionIndex) => {
                                                ...
                                                const isWinnerSeat = hasWinner && seatNum === winningSeat;
                                                const componentToRender = getComponentToRender(position, positionIndex);

                                                return (
                                                    <div key={positionIndex} className="z-[10] seat-wrapper" style={{ left: position.left, top: position.top }}>
                                                        {/* 1. Turn indicator (only if no winner yet) */}
                                                        {!hasWinner && <MemoizedTurnAnimation index={positionIndex} />}

                                                        {/* 2. Winner ripple (only on the actual winning seat) */}
                                                        {isWinnerSeat && <WinAnimation index={positionIndex} />}

                                                        {/* 3. Actual Player or VacantPlayer */}
                                                        {componentToRender}

                                                        {/* ─── SEAT‐CHIP: only when showChip && moveChip && isWinnerSeat ─── */}
                                                        <AnimatePresence>
                                                            {showChip && moveChip && isWinnerSeat && (
                                                                <motion.div
                                                                    key={`seatChip-${seatNum}`}
                                                                    layoutId="winningChip"
                                                                    initial={{ opacity: 1, scale: 0.8 }}
                                                                    animate={{ opacity: 1, scale: 1 }}
