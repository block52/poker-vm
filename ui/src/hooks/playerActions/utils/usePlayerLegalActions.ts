...
export function usePlayerLegalActions(tableId?: string): PlayerLegalActionsResult {
    ...
    try {
        ...
        if (userAddress && gameState.players?.length > 0) {
            ...
            console.log(
                "⚠️ Players in game:",
                gameState.players.map((p: any) => `Seat ${p.seat}: ${p.address?.toLowerCase()}`)
            );
        ...
        if (Array.isArray(currentPlayer.legalActions) && currentPlayer.legalActions.length > 0) {
            ...
            const allSameIndex = currentPlayer.legalActions.every((action: LegalAction) => action.index === firstActionIndex);

            if (!allSameIndex) {
                console.warn("⚠️ WARNING: Not all legal actions have the same index!");
                console.warn(
                    "⚠️ Action indices:",
                    currentPlayer.legalActions.map((a: LegalAction) => `${a.action}: ${a.index}`)
                );
            }
