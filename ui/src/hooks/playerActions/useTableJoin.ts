...
async function joinTableFetcher(url: string, { arg }: { arg: JoinTableOptions }) {
    const { buyInAmount, userAddress, privateKey, publicKey, nonce = Date.now().toString(), actionIndex = 0, seatNumber } = arg;

    if (!userAddress || !privateKey) {
        throw new Error("Missing user address or private key");
    }

    console.log("Joining table with parameters:", {
        actionIndex,
        seatNumber: seatNumber !== undefined ? seatNumber : "auto-assign"
    });
...
export function useTableJoin(tableId: string | undefined) {
    const { trigger, isMutating, error, data } = useSWRMutation(tableId ? `${PROXY_URL}/table/${tableId}/join` : null, joinTableFetcher);
