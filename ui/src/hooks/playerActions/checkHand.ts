 ...
 */
export async function checkHand(tableId: string, network: NetworkEndpoints): Promise<any> {
    ...
    // Call SigningCosmosClient.performAction() with "check" action
    const transactionHash = await signingClient.performAction(
        tableId,
        "check",
        0n // Check doesn't require an amount
    );
