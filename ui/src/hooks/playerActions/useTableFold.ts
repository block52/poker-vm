...
export function useTableFold(tableId: string | undefined) {
    const { trigger, isMutating, error, data } = useSWRMutation(tableId ? `${PROXY_URL}/table/${tableId}/fold` : null, foldFetcher);
    ...
    const result = {
        foldHand: tableId ? (options: Omit<FoldOptions, "actionIndex"> & { actionIndex?: number | null }) => trigger(options) : null,
