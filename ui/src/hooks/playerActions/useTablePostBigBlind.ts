...
async function postBigBlindFetcher(url: string, { arg }: { arg: PostBigBlindOptions }) {
...
export function useTablePostBigBlind(tableId: string | undefined) {
    ...
    const { trigger, isMutating, error, data } = useSWRMutation(tableId ? `${PROXY_URL}/table/${tableId}/post_big_blind` : null, postBigBlindFetcher);
