...
abstract class BaseAction {
    ...
    verify(player: Player): Range | undefined {
        // 1. Check if hand is still active (not in showdown)
        if (this.game.currentRound === TexasHoldemRound.SHOWDOWN) 
            throw new Error("Hand has ended.");

        // 2. Check if it's player's turn (except for fold which can be done anytime)
        ...
        // 3. Check if player is still active (not folded/all-in)
