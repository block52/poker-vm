...
main = do
    ...
    -- Serialization tests (TypeScript compatibility)
    runTest "Deck to string" testDeckToString
...
-- Serialization tests (TypeScript compatible)
testDeckToString :: Bool
testDeckToString =
    let deck = newDeck
        str = deckToString deck
        -- Should start with AC (Ace of Clubs)
        startsCorrect = take 2 str == "AC"
        -- Should have 51 dashes (52 cards - 1)
        dashCount = length $ filter (== '-') str
    in startsCorrect && dashCount == 51

testDeckFromString :: Bool
testDeckFromString =
    let deckStr = "AC-2C-3C-4C-5C-6C-7C-8C-9C-TC-JC-QC-KC-" ++
                  "AD-2D-3D-4D-5D-6D-7D-8D-9D-TD-JD-QD-KD-" ++
                  "AH-2H-3H-4H-5H-6H-7H-8H-9H-TH-JH-QH-KH-" ++
                  "AS-2S-3S-4S-5S-6S-7S-8S-9S-TS-JS-QS-KS"
    in case deckFromString deckStr of
        Just deck -> remaining deck == 52
        Nothing -> False

testDeckStateWithPosition :: Bool
testDeckStateWithPosition =
    let deckStr = "[AC]-2C-3C-4C-5C-6C-7C-8C-9C-TC-JC-QC-KC-" ++
                  "AD-2D-3D-4D-5D-6D-7D-8D-9D-TD-JD-QD-KD-" ++
                  "AH-2H-3H-4H-5H-6H-7H-8H-9H-TH-JH-QH-KH-" ++
                  "AS-2S-3S-4S-5S-6S-7S-8S-9S-TS-JS-QS-KS"
    in case deckStateFromString deckStr of
        Just ds -> dsTop ds == 0 && length (dsCards ds) == 52
        Nothing -> False

testRoundtripSerialization :: Bool
testRoundtripSerialization =
    let deck = shuffleDeck 42 newDeck
        ds = mkDeckState deck
        -- Draw a few cards
        ds' = case getNext ds of
            Just (_, ds1) -> case getNext ds1 of
                Just (_, ds2) -> ds2
                Nothing -> ds1
            Nothing -> ds
        -- Serialize and deserialize
        serialized = deckStateToString ds'
    in case deckStateFromString serialized of
